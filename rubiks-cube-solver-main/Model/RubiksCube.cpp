...
// Helper: returns string of corner colors
...
uint8_t RubiksCube::getCornerOrientation(uint8_t ind) const {
    ...
    for (auto c : corner) {
        if (c == 'W' || c == 'Y') actual_str.push_back(c);
    }
    if (actual_str.empty()) return 0;
